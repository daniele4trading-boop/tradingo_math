...
PARAM_SCHEMA: dict[str, dict[str, type | tuple[type, ...]]] = {
    ...
    "execution": {
        ...
        "dry_run": bool,
        "comment_prefix": str,
        "assumed_move_pct_for_sizing": (int, float),
    },
    "backtest": {
        "initial_equity": (int, float),
        "lot_a": (int, float),
        "lot_min": (int, float),
...
def validate_params(params: dict[str, Any]) -> None:
    ...
    if execution["deviation_points"] < 0:
        raise ConfigError("params.json: deviation_points deve essere >= 0")
    if execution["assumed_move_pct_for_sizing"] <= 0:
        raise ConfigError("params.json: assumed_move_pct_for_sizing deve essere > 0")
    backtest = params["backtest"]
    if backtest["initial_equity"] <= 0:
        raise ConfigError("params.json: backtest.initial_equity deve essere > 0")
    if backtest["lot_a"] <= 0 or backtest["lot_min"] <= 0:
        raise ConfigError("params.json: backtest lot_a e lot_min devono essere > 0")
    if backtest["lookback_days"] < 0:
        raise ConfigError("params.json: backtest.lookback_days deve essere >= 0")
    launch = params["launch"]
    if launch["max_executions_per_cycle"] < 0:
        raise ConfigError("params.json: max_executions_per_cycle deve essere >= 0")
    if launch["cycle_interval_minutes"] <= 0:
        raise ConfigError("params.json: cycle_interval_minutes deve essere > 0")
